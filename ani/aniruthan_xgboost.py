...
def feature_engineering_train(df):
    ...
    df['open_mean_5'] = df['Open'].rolling(5).mean()
    df['close_mean_5'] = df['Close'].rolling(5).mean()
    df['open_std_5'] = df['Open'].rolling(5).std()
    df['close_std_5'] = df['Close'].rolling(5).std()

    df['open_mean_20'] = df['Open'].rolling(20).mean()
    df['close_mean_20'] = df['Close'].rolling(20).mean()
    df['open_std_20'] = df['Open'].rolling(20).std()
...
model = XGBRegressor(
    n_estimators= 1000,         # TODO: Try different values
